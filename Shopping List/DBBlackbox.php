...
class DBBlackbox
{
     ...
     */
    public static function update($id = null, array $data = [])
    {
        static::load();
        if(!$id)
        {
     ...
     */
    public static function select($limit = null, $offset = null)
    {
        static::load();
        if($limit || $offset)
        {
            return array_slice(static::$data, intval($offset), $limit ?: null, true);
        }
        return static::$data;
    }
    /**
...
{
    return DBBlackbox::select($limit, $offset);
}
function find($id)
{
    return DBBlackbox::find($id);
}